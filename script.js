...
function initializeScrollEffects() {
  ...
  const animateElements = document.querySelectorAll(
    ".service-card, .doctor-card, .about-text, .about-image, .contact-info, .contact-form"
  );
...
function initializeFormHandling() {
  ...
  if (contactForm) {
    contactForm.addEventListener("submit", (e) => {
      ...
      showNotification(
        "Xabaringiz uchun rahmat! Tez orada siz bilan bogʻlanamiz.",
        "success"
      );
      contactForm.reset();
    });
...
setTimeout(enhanceFloatingCards, 5000);
